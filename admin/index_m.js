...
function parseTokens(tokensStr) {
    if (!tokensStr || typeof tokensStr !== 'string') {
        return { tizen: {}, hj: {} };
    }
    try {
...
function load(settings, onChange) {
    ...
    initSelects();
    $(document)
        .off('change keyup', '.input-field input', activateLabels)
...
function isAlreadyAdded(discoveredDevice) {
    ...
    return devices.some(dev => {
        ...
        if (dmac && normalizeMacInput(dev.mac || '') === dmac) {
            return true;
        }
        if (!did && !dmac && dip && dev.ip === dip) {
            return true;
        }
        return false;
    });
...
function removeDevice(idx) {
    ...
    if (dev && dev.id) {
        if (tokens.tizen[dev.id]) {
            delete tokens.tizen[dev.id];
        }
        if (tokens.hj[dev.id]) {
            delete tokens.hj[dev.id];
        }
    }
...
function isDevicePaired(dev) {
    ...
    if (dev.api === 'hj') {
        return !!tokens.hj[dev.id];
    }
    if (dev.api === 'legacy') {
        return false;
    }
    if (dev.api === 'unknown') {
        return !!tokens.tizen[dev.id] || !!tokens.hj[dev.id];
    }
    ...
    try {
        ...
        if (pdoc && typeof MutationObserver !== 'undefined') {
            ...
            if (pdoc.documentElement) {
                targets.push(pdoc.documentElement);
            }
            if (pdoc.body && pdoc.body !== pdoc.documentElement) {
                targets.push(pdoc.body);
            }
            targets.forEach(t =>
                observer.observe(t, {
                    attributes: true,
...
function detectTheme() {
    ...
    try {
        ...
        if (params.has('react')) {
            theme = `react-${params.get('react')}`;
        }
        if (!theme && params.has('theme')) {
            theme = `react-${params.get('theme')}`;
        }
        if (!theme && params.has('dark')) {
            theme = 'react-dark';
        }
    } catch (e) {
        // ignore
        ...
        try {
            ...
            if (pdoc) {
                ...
                if (combined.includes('dark')) {
                    theme = 'react-dark';
                } else if (combined.includes('blue')) {
                    theme = 'react-blue';
                } else if (combined.includes('colored')) {
                    theme = 'react-colored';
...
function detectDarkByBackground() {
    try {
        ...
        if (!el || !window.parent.getComputedStyle) {
            return false;
        }
